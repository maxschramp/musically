...
async function init() {
  const { defaultQuality, targetSubfolder } = await browser.storage.local.get([
...
qSel.addEventListener("change", () => {
  browser.storage.local.set({ defaultQuality: qSel.value });
});

subEl.addEventListener("change", () => {
  browser.storage.local.set({ targetSubfolder: subEl.value.trim() });
});
...
btn.addEventListener("click", async () => {
  ...
  btn.disabled = true;
  try {
    // Persist current subfolder before download so background reads latest.
    await browser.storage.local.set({ targetSubfolder: subEl.value.trim() });
    const [tab] = await browser.tabs.query({ active: true, currentWindow: true });

...
(() => {
  ...
  function pageEntity() {
    // /album/<id>, /track/<id>, /playlist/<id> from current URL.
  ...
  function inject(popover) {
    ...
    a.addEventListener("click", async (ev) => {
      ...
      try {
        ...
        const r = resp.result;
        render(r.kind === "album" ? `Queued ${r.count}/${r.total}` : "✓ Downloading");
      } catch (e) {
  ...
  const observer = new MutationObserver((mutations) => {
    for (const m of mutations) {
      if (m.type === "childList") {
        ...
        // Strip our stale items so we re-inject for the new id/kind.
